...
> Swift code: `Sources/NovaSwiftKit/JunkModels.swift` (`JunkRes`, decoding `jünk`)
> and `Sources/NovaSwiftKit/OopsModels.swift` (`OopsRes`, decoding `öops`) both
...
> `NovaSwiftKit` calls `junk()`/`junks()`/`oops()`/`oopses()` yet, so junk trading
