...
| `NewsGovt1-4`/`GovtNewsStr1-4` (per-government local news) | ❌ Not decoded, and real data uses it — **exact byte layout now confirmed** | `CronRes` (`MissionModels.swift:264-305`) only reads bytes `0`–`788`. The 789-byte assumption itself has a bug, not just a missing tail: it treats all three NCB strings (`EnableOn`/`OnStart`/`OnEnd`) as 255 bytes each, but the `crön` TMPL (`third_party/ResForge/Plugins/Sources/NovaTools/Templates.rsrc`, TMPL #503, decoded via `evnova-extract tmpl`) types them `n0FF`/`n0FF`/`n100` — 255/255/**256** bytes (ResForge's `Xnnn` convention reads the suffix as hex; `n100` = 0x100 = 256, one byte longer than the other two). Re-deriving the full layout from the TMPL's field sizes and confirming against a raw hex dump of `crön #128` "Wraith Change" (`swift run evnova-extract raw "data/EV Nova" crön 128`, 822 bytes) matches byte-for-byte: `firstDay@0,firstMonth@2,firstYear@4,lastDay@6,lastMonth@8,lastYear@10,availRandom@12,duration@14,preHoldoff@16,postHoldoff@18,indNewsStr@20,flags@22` (=24-byte head, unchanged), `enableOn@24`(255B)`,onStart@279`(255B)`,onEnd@534`(**256B**)` → 790, `contribute@790`(8B)`,require@798`(8B)` → 806, `newsGovt1-4@806`(4×2B)`,govtNewsStr1-4@814`(4×2B)` → 822. This lands `NewsGovt1`/`GovtNewsStr1` at exactly the two nonzero offsets already spotted in cron #128 — **806 (=130, a real govt id) and 814 (=15000, a real `STR#` id)** — confirming both the field identity and the offset. Total: 822 bytes, matching every real cron exactly (cron #129 "Terraforming Start" has an all-zero tail at these same offsets, consistent with an unused feature, not a wrong offset). |
